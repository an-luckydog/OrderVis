...
def get_accuracy(data):
    ...
    for i in range(len(data)):
        ...
        if res < 0.5:
            acc_before_count += 1
        for j in range(len(data[i]["reorderRes"][0])):
            # if len(data[i]["reorderRes"][j]) <= 1:
            #     fail_count += 1
            #     continue
            count += 1
            if data[i]["reorderRes"][0][j][0] < 0.5:
                acc_after_count += 1
...
def get_flip_ratio(data):
    ...
    for i in range(len(data)):
        res = data[i]["res"][0]
        print("原始res：", res)
        # print(len(data[i]["reorderRes"][0]))
        for j in range(len(data[i]["reorderRes"][0])):
            flag = -1
            if res > 0.5:
                if data[i]["reorderRes"][0][j][0] < 0.5 :
                    flag = 1
                    print("翻转res：", j, data[i]["reorderRes"][0][j][0])
                else:
                    flag = 0
            else:
                if data[i]["reorderRes"][0][j][0] > 0.5 :
...
def get_couple_importance(data):
    ...
    for i in range(len(data)):
        ...
        print("原始res：", res)
        for j in range(len(data[i]["reorderRes"][0])):
            # if len(data[i]["reorderRes"][0][j]) == 1:
            #     continue
            count += 1
            diff_sum += abs(data[i]["reorderRes"][0][j][0] - res)
    # print ("couple importance：", diff_sum / (data_len * 10 - fail_count))
    return diff_sum / count

    
if __name__ == '__main__':
    file_path = 'test_imdb_two.json'
